...
while True:
    ...
    if contours:
        ...
        if M["m00"] != 0:
            ...
            error = apply_dead_zone(error, 10)  # Apply a dead zone of 10 pixels
            current_time = cv2.getTickCount()
            dt = (current_time - prev_time) / cv2.getTickFrequency()  # Time difference in seconds
            steering_angle = pid_controller.update(error, dt)
            steering_angle = steering_average.update(steering_angle)
            prev_time = current_time
            
            # Determine direction based on steering angle
            if abs(steering_angle) < 15:  # Increased threshold for going straight
                print("Go Straight")
